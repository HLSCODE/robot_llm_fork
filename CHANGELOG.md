...
- 修复弹框、启动卡片和下拉菜单查询屏幕后，PySide/Shiboken 在临时窗口回收时提前销毁共享屏幕导致的闪退；统一使用应用级屏幕查询并返回几何信息副本，保留多屏定位。
- Windows 自动原生故障日志改为仅输出当前 Python 线程，避免故障处理器遍历其他线程时发生二次崩溃；保留手动全线程快照和 CDB/ProcDump 完整转储。

### Added

- 天机轨迹录制后支持命名入库、导入已有 FMV 和工作流回放；新版 SDK 区分正常完成与按钮中断，中断不继续后续动作。
...
- 初始化依赖时自动识别 uv 缓存与项目是否跨文件系统，并在必要时使用 copy 链接模式。
