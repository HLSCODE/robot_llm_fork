...
> 实施进度：D-015～D-021 已完成；纯 WorkflowDocument、版本化 `.workflow`
...
本次 GUI 重构应以“受约束的工作流画布”替换当前列表式编排区域，并继续复用项目已经收敛的领域模型、应用服务、设备运行时和唯一执行运行时。
...
新节点自动布局并连接。拖动节点默认表示排序；只有进入高级连线模式时才允许直接操作边，避免窄屏触控中“移动节点”和“拖线”冲突。

### 6.2 选择与编辑

- 单击选中；Ctrl/Shift 或框选进行多选。
- 双击或按 Enter 打开完整参数编辑器。
- 右侧参数抽屉提供稳定编辑区域；悬停仅提供只读摘要，不能承载关键操作。
...
- 鼠标滚轮缩放，空格+拖动或中键平移。
- 触控支持双指缩放、双指平移；单指只在命中节点后拖动节点，空白区域单指平移。
...
本计划完成不等于“画布能显示”。只有在单一数据源、单一执行入口、现有功能等价、触控与可访问性、性能、异常恢复以及安全控制均通过验收，并删除旧编辑入口后，GUI 重构才可标记完成。
