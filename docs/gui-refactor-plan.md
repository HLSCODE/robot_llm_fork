...
> 实施进度：D-015～D-021 已完成；D-022～D-025 已立项。纯 WorkflowDocument、版本化 `.workflow`
...
- 建立 Qt Resource/SVG 图标体系和许可证清单，完成深色、浅色、系统主题与高 DPI 验证。
- 持久化 Side Bar/Bottom Panel 尺寸、可见性和当前页面，损坏状态必须安全恢复默认布局。
- 完成尺寸矩阵、触控、键盘、读屏、功能等价和安全回归后直接切换并删除旧布局组件。
