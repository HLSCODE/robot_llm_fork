...
> 2026-08-06 交互复核：节点间及循环体内“+”均已接入真实插入命令；动作库
> 抽屉在左侧分隔边缘提供常驻箭头和 220 ms 过渡动画；Loop 改为头部、展开
> 子动作、完成节点及双侧回路的结构化表达；画布采用常规桌面编辑器手势。
...
动作库抽屉除保留分隔条调整宽度外，必须在左侧面板与画布之间的分隔边缘提供常驻箭头按钮；不得占用画布上方工具栏。开合使用约 220 ms 的缓入缓出动画，并记住最近一次展开宽度，避免要求用户精确拖动窄小分隔条。

分隔边缘默认宽度收敛到 4 px 并只绘制一条细线；悬停时实际展开到 28 px，整条命中带显示方向箭头。整条高度均可单击开合，也必须支持水平拖动实时调整动作库宽度，拖动结果作为下次展开宽度。动作插入选择器采用“动作类型—分类内动作”双栏结构，空分类不显示，禁止把所有动作重新扁平化为单个长列表。
