...
录制协议与回放协议独立；旧 `TrajectoryControl` 作为兼容组合接口保留，RealMan
继续使用发送后轮询的 `TrajectoryPlayback`。Tianji 声明 `trajectory_recording` 和
`trajectory`，通过 `BlockingTrajectoryPlayback.execute_trajectory()` 在执行线程调用
`run_trajectory(is_block=True)`，只有整数 `0` 才视为完成；`1` 转为动作取消，工作流
不继续下一步；未知返回值（包括旧 SDK 的 `None`）按失败处理。

在“轨迹类”点击新增并录制后，会询问动作名称，将所选臂的 FMV 路径写入动作库。
左右臂分别使用 `*_L.fmv` / `*_R.fmv`，其余原始数据保留，不作为回放输入。
已有录制无需重录：新增时选择“使用已有文件”，选择对应机械臂及 FMV，命名即可。
位于该臂轨迹目录下的嵌套文件直接引用，不重复复制。动作仍受 Robot Profile 隔离。
...
- 阻塞运动及回放内部没有公开超时/主动取消参数。按钮中断已能返回 `1`（起点阶段
  中断不会继续启动回放），但软件停止后 SDK 等待不一定退出。调用在执行线程进行，
  返回前不释放设备资源；动作超时/取消在调用返回后检查，不伪装成可强制终止线程。
