...
class ManagedBalanceCameraCapture:
    ...
    def __init__(
        ...
        *,
        ...
        self._camera_name = camera_name.strip()
    ...
    def _wait_for_frame(self, camera: CameraSource) -> bytes:
        ...
        while True:
            for serial, name, jpeg in camera.get_latest_jpegs():
                if not jpeg:
                    continue
                if self._camera_name and self._camera_name not in (serial, name):
                    continue
                return bytes(jpeg)
            if self._clock() >= deadline:
                label = self._camera_name or "任意已配置相机"
                raise TimeoutError(f"等待电子秤相机画面超时: {label}")
    ...
    *,
    ...
    if simulation:
        ...
        capture = ManagedBalanceCameraCapture(
            camera_access,
            camera_name=settings.balance_camera_name,
